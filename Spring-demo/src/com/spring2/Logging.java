...
public class Logging {
    ...
    @Before("execution(* com.spring2.*.*(..))")
    public void beforeAdvice(){
        System.out.println("1. Going to setup student profile.");
    }
    @After("execution(* com.spring2.*.*(..))")
    public void afterAdvice(){
        System.out.println("2. Student profile has been setup.");
    }
    @AfterReturning(pointcut = "execution(* com.spring2.*.*(..))", returning = "retVal")
    public void afterReturningAdvice(Object retVal){
        System.out.println("3. Returning: " + retVal.toString() );
    }
    @AfterThrowing(pointcut = "execution(* com.spring2.*.*(..))", throwing = "ex")
